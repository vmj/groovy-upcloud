...
trait ServerApi {
     ...
     */
    def start(...args) {
        HTTP.POST(cmdPath('start'), null, *args)
    }
    ...
    def stop(...args) {
        ...
        HTTP.POST(cmdPath('stop'), stop, *args)
    }
    ...
    def restart(...args) {
        ...
        HTTP.POST(cmdPath('restart'), restart, *args)
    }
     ...
     */
    def eject(...args) {
        HTTP.POST(cdromPath('eject'), null, *args)
    }
    ...
    def firewallRules(...args) {
        HTTP.GET(firewallRulesPath(), *args)
    }
    ...
    def loadFirewallRule(def position, ...args) {
        HTTP.GET(firewallRulePath(position), *args)
    }
    ...
    def deleteFirewallRule(def position, ...args) {
        HTTP.DELETE(firewallRulePath(position), *args)
    }
    ...
    def addTags(def tags, ...args) {
        HTTP.POST(tagPath(tags), null, *args)
    }
    ...
    def deleteTags(def tags, ...args) {
        HTTP.POST(untagPath(tags), null, *args)
    }
